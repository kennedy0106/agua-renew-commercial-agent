...
- **Graders determinísticos** (`eval/lib/graders.mjs`): 16 métricas (precio, mínimo, políticas, memoria, no repetición, herramientas, siguiente paso, no cierre prematuro, información restringida, canal, tono, brevedad, naturalidad, orientación comercial, objeciones) con escala 0/1/2 y umbrales por métrica. Las blandas usan proxies determinísticos (LLM juez queda reservado con `--judge`).
...
- **Baseline DeepSeek Flash V4** (dataset v1, 81 escenarios): score global **96.5/100**, 1 critical failure (F006: el modelo discute márgenes/rentabilidad — término restringido que el guard de texto no cubre), precio/mínimo/política 100%, no repetición 100%, sin cierre prematuro 100%. Debilidades medidas: memoria del prospecto no persistida en turnos sociales (81.5%), selección de herramientas 86.4% (no llama herramienta esperada en algunos flujos), hechos esperados 80.2% (p. ej. `get_quote` de bidón sin `purchaseType`; fuga de razonamiento en inglés en E006), tuteo esporádico en saludos (96.3%). Latencia turno p50 ~4.0s / p90 ~4.9s. Costo ~USD 0.0033/conversación (~USD 268/1000 conv con pricing referencial). GPT-5.6 Luna: adaptador listo, **not executed** (sin OPENAI_API_KEY).
