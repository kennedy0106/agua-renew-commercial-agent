...
- **Baseline DeepSeek Flash V4** (dataset v1, 81 escenarios): score global **95.8/100 con grader v2** (97 con grader v1 — el re-grado del mismo transcript, sin nueva llamada API, es la medición calibrada), **3 critical failures**: E009 (fuga de protocolo: "La herramienta requiere un purchaseType"), E010 (cotiza `distribution_bidon_20l_new` S/20 en vez de maquila — producto equivocado con precio válido) y M005 (afirma "producción continua"/stock). Precio/mínimo/política/restringido 100%, no repetición 100%, sin cierre prematuro 100%. Debilidades medidas: memoria del prospecto no persistida en turnos sociales (82.7%), selección de herramientas 88.9%, hechos esperados 85.2%, orientación comercial 86.4%, grounded claims 96.3%, resolución de producto 97.5%, protocol leak 98.8%. Latencia turno p50 ~3.7s / p90 ~5.3s. Costo ~USD 0.0033/conversación (~USD 3.3/1000 conv con pricing referencial). GPT-5.6 Luna: adaptador listo, **not executed** (sin OPENAI_API_KEY).
